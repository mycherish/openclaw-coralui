...
- 🎨 **Modern UI**: macOS-style design with smooth animations, native traffic lights and draggable title bar
  现代界面 - macOS 风格设计，流畅动画，原生红绿灯和可拖动标题栏

- 📊 **Dashboard**: Real-time system status monitoring (gateway, health, nodes, models, channels)
  仪表板 - 实时系统状态监控（网关、健康、节点、模型、渠道）
...
- 🎯 **System Tray**: Quick access from system tray (show/hide window, Quick Chat, settings)
  系统托盘 - 从系统托盘快速访问（显示/隐藏窗口、Quick Chat、设置）

- ⌨️ **Quick Chat**: Floating chat window with global shortcut (`Cmd/Ctrl+Shift+O`)
  Quick Chat - 浮动聊天窗口，支持全局快捷键（`Cmd/Ctrl+Shift+O`）

- 🚀 **Auto Launch**: Option to start automatically on system boot
  开机自启 - 可选择开机自动启动

- ⚙️ **Settings Persistence**: All settings saved locally and persist across sessions
  设置持久化 - 所有设置本地保存，跨会话持久化

- 🎹 **Customizable Shortcuts**: Record and customize Quick Chat shortcut
  可自定义快捷键 - 录制和自定义 Quick Chat 快捷键
...
| `npm run electron:build -- --mac` | Build for macOS | 构建 macOS 版本 |
| `npm run electron:build -- --win` | Build for Windows | 构建 Windows 版本 |
| `npm run electron:build -- --linux` | Build for Linux | 构建 Linux 版本 |
...
| Shortcut | Action | 操作 |
|----------|--------|------|
| `Cmd/Ctrl + Shift + O` | Toggle Quick Chat window | 切换 Quick Chat 窗口 |
| `Cmd/Ctrl + Q` | Quit application (macOS) | 退出应用 (macOS) |
| `Cmd/Ctrl + W` | Close window | 关闭窗口 |
